...
public sealed class OnionLayout : ArchitectureLayoutBase
{
    ...
    public override ImmutableArray<ProjectDefinition> Projects(ApiSmithConfig config)
    {
        ...
        var servicesRefs = CsprojTemplates.ProjectReferencesBlock($"../{domain}/{domain}.csproj");
        ...
        return ImmutableArray.Create(
            ...
            new ProjectDefinition(services, $"src/{services}/{services}.csproj",
                CsprojTemplates.ClassLibrary(config, services, services, servicesRefs),
    ...
    public override string DtoPath(ApiSmithConfig c, string schema, string fileName)   => $"src/{Services(c)}/Dtos{SchemaFolderSegment(c, schema)}/{fileName}.cs";
    ...
    public override string DtoNamespace(ApiSmithConfig c, string schema)       => $"{Services(c)}.Dtos{SchemaNamespaceSegment(c, schema)}";
